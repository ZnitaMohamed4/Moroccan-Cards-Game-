...
{ // cette foncion utilise QTimer::singleShot pour simuler le delai entre chaque distribution de carte.
...
// Assuming the header files are properly included
  ...
  void Facile::changeBoutonJouer(QPushButton *BoutonCarte)
  {
      ...
      //cette fonction permet de synchroniser visuellement le bouton du joueur avec la carte selectionnee,
      //en mettant a jour son image de fond en fonction de la carte cliquee.
...
// Cette fonction permet de prendre l'image d'une carte qui est dans un bouton
  ...
  void Facile::gestionScore()
 {
      ...
      {
      // Ranks are equal, update player_score and comp_score
      int currentPlayerScore = score_jouer->text().split(":").back().trimmed().toInt();
      score_jouer->setText("SCORE JOUER: " + QString::number(currentPlayerScore + 1));
 ...
 void Facile::jeuxfini()
 {
      ...
      // Check if 30 comparisons have been reached
      ...
      {
          winnerMessage = "Jeux Fini! ORDINATEUR Gagnant!";
      }
