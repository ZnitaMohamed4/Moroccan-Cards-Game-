...
Diffcile::Diffcile(QWidget *parent)
    : QWidget (parent)
{
    ...
    place_jeux->setStyleSheet("QWidget {"
                              ...
                              "border-radius: 15px;"
                              "background-image: url(:/picture/table.jpg);"
    ...
    connect(bouton_deck, &QPushButton::clicked, this, &Diffcile::BoutonDeckClicke);
    ...
    connect(this, &Diffcile::checkGameFini, this, &Diffcile::jeuxfini);

}

void Diffcile::BoutonDeckClicke()
{
...
void Diffcile::changerCarte(QPushButton *button, const Carte &carte)
{
...
void Diffcile::changeBoutonJouer(QPushButton *BoutonCarte)
{
...
void Diffcile::updateBoutonOrdi()
{
...
Carte Diffcile::getCardFromButton(QPushButton *cardButton)
{
...
Rank Diffcile::getCardRank(QPushButton *cardButton)
{
    Carte carte = getCardFromButton(cardButton);
    return carte.getCardRank();
}

QPushButton* Diffcile::findMatchingCard(Rank targetRank)
{
...
void Diffcile::gestionScore()
{
    ...
    if (compCard1 == compCard2 && playerCard1 != playerCard2 || compCard1 == playerCard2)
    {
        // Ranks are equal, update comp_score
        int currentCompScore = score_ordi->text().split(":").back().trimmed().toInt();
        score_ordi->setText(" SCORE ORDI : " + QString::number(currentCompScore + 2));
...
void Diffcile::updateUIAfterScore()
{
    bouton_ordi->setStyleSheet("background-color: green;");
    bouton_jouer->setStyleSheet("background-color: green;");
}
